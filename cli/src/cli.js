...
program
    .version(version)
    .description('Openbotfront CLI')
    .action(() => console.log(`\n${chalk.red.bold('ERROR:')} Unsupported command. Run ${chalk.cyan.bold('openbotfront --help')} for more information.\n`));
...
program
    ...
    .description('Create a new Openbotfront project.')
    .action(initCommand);
...
program
    ...
    .option('--ci', 'No spinners, no prompt confirmations')
    .description('Start a Openbotfront project.  Must be executed in your project\'s directory')
    .action(dockerComposeUp);

program
    .command('down')
    .option('-v, --verbose', 'Display Docker Compose start-up logs')
    .description('Stops a Openbotfront project and releases Docker resources.  Must be executed in your project\'s directory')
    .action(dockerComposeDown);

program
    .command('logs')
    .option('--ci', 'Print out logs once and do not hook to TTY')
    .description('Display openbotfront logs. Must be executed in your project\'s directory')
    .action(dockerComposeFollow);

program
    .command('killall')
    .option('--remove-images', 'Will also remove Openbotfront related Docker images')
    .description('Stops any running Openbotfront project')
...
program
    ...
    .description('Stop a Openbotfront service (interactive). Must be executed in your project\'s directory')
    .action(dockerComposeStop);

program
    .command('start <service>')
    .description('Start a Openbotfront service (interactive). Must be executed in your project\'s directory')
    .action(dockerComposeStart);

program
    .command('restart <service>')
    .description('Restart a Openbotfront service (interactive). Must be executed in your project\'s directory')
...
program
    .command('update')
    .description('Update a project if the current Openbotfront version as a higher minor version but not a different major version')
...
async function killAllCommand(cmd) {
    const { stop } = await inquirer.prompt({
        type: 'confirm',
        name: 'stop',
        message: 'This will stop any running Openbotfront project and cleanup remaining Docker resources. This will not affect your project\'s data. Proceed ?',
        default: true,
    });
    if (stop){
        const spinner = ora();
    ...
    try {
        ...
        if (isProjectDir()){
            if (containers && containers.length){
                choices.push({ title: 'Stop Openbotfront', cmd: () => dockerComposeDown({ verbose: false }) });
            ...
            if (isMinorUpdate()){
                choices.push({ title: `Update project to Openbotfront ${getBotfrontVersion()}`, cmd: doMinorUpdate});
            }
        } else {
            if (containers && containers.length){
                choices.push({ title: 'Stop Openbotfront', cmd: () => killAllCommand({ verbose: false }) });
            }
            choices.push({ title: 'Create a new project', cmd: initCommand });
        }
        choices.push({ title: 'Browse the online documentation', cmd: openDocs});
        choices.push({ title: 'More options (display the --help)', cmd: () => shell.exec('openbotfront -h') });
        choices.push({ title: 'Exit', cmd:  () => process.exit(0) });
        console.log(boxen(`Welcome to ${chalk.green.bold('Openbotfront')}!\nversion: ${getBotfrontVersion()}`,  { padding: 1,  margin: 1 }));
